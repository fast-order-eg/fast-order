<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">تعديل منتج</h2>
    </x-slot>

    <div class="py-6">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <form method="POST" action="{{ route('products.update', $product) }}" class="text-right" enctype="multipart/form-data" id="productEditForm">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <select name="category_id" class="w-full border rounded p-2 appearance-none bg-[length:16px_16px] bg-[left_0.75rem_center] pl-8 pr-2" style="background-image:url('data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'16\' height=\'16\' viewBox=\'0 0 20 20\' fill=\'%23666\'><path d=\'M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z\'/></svg>');">
                ...
                <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1">السعر قبل</label>
                        <input type="number" step="0.01" name="price_before" value="{{ old('price_before', $product->price_before) }}" class="w-full border rounded p-2">
                        @error('price_before')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="block mb-1">السعر بعد</label>
                        <input type="number" step="0.01" name="price_after" value="{{ old('price_after', $product->price_after ?: $product->price) }}" class="w-full border rounded p-2">
                ...
                <div class="mb-4 p-4 border border-blue-100 bg-blue-50 rounded-lg">
                    ...
                    <input type="hidden" name="price_tiers_json" id="price_tiers_json" value="{{ old('price_tiers_json', $product->price_tiers) }}">
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <label class="block mb-1 font-medium">نوع الشحن</label>
                    <div class="space-y-3">
                        <div class="flex items-center">
                            <input type="radio" id="free_shipping" name="shipping_type" value="free" 
                                   ...
                                   {{ old('shipping_type', $product->shipping_type ?? 'free') == 'free' ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <input type="radio" id="governorate_shipping" name="shipping_type" value="governorate" 
                                       class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                       {{ old('shipping_type', $product->shipping_type ?? 'free') == 'governorate' ? 'checked' : '' }}>
                ...
                @php
                    ...
                    $existingColors = $product->colors ? json_decode($product->colors, true) : [];
                @endphp
                <div class="mb-4">
                    <div class="flex items-center mb-2">
                        <input type="checkbox" id="enableSizes" class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded ml-2" {{ !empty($existingSizes) ? 'checked' : '' }}>
                        <label for="enableSizes" class="font-medium">المقاسات</label>
                    </div>
                    <div id="sizesContainer" class="{{ empty($existingSizes) ? 'hidden' : '' }}">
                        <div id="sizesList" class="space-y-2 mb-2">
                            @foreach($existingSizes as $size)
                            <div class="flex items-center gap-2">
                                <input type="text" name="sizes[]" value="{{ $size }}" placeholder="مثال: L, XL, 42" 
                ...
                <div class="mb-4">
                    <div class="flex items-center mb-2">
                        <input type="checkbox" id="enableColors" class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded ml-2" {{ !empty($existingColors) ? 'checked' : '' }}>
                        <label for="enableColors" class="font-medium">الألوان</label>
                    </div>
                    <div id="colorsContainer" class="{{ empty($existingColors) ? 'hidden' : '' }}">
                        <div id="colorsList" class="space-y-2 mb-2">
                            @foreach($existingColors as $color)

                            <div class="flex items-center gap-2">
                                <input type="text" name="colors[]" value="{{ $color }}" placeholder="مثال: أحمر, أزرق, أسود" 
                                       class="flex-1 border rounded-lg p-2 focus:ring-2 focus:ring-indigo-200">
                                <button type="button" onclick="removeItem(this)" 
                                        class="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                ...
                <div class="mb-4 mt-6 border-t pt-4">
                    <input type="hidden" name="variants_stock" id="variants_stock_json" value="{{ old('variants_stock', $product->variants_stock) }}">
                ...
                <div class="mb-4">
                    <label class="block mb-1">الصورة الرئيسية (اختياري)</label>
                    @if($product->main_image_path)
                        <div class="mb-2"><img src="{{ asset('storage/'.$product->main_image_path) }}" alt="الصورة الحالية" class="h-24 w-24 object-cover rounded border"></div>
                    @endif
                    <input type="file" name="main_image" accept="image/*" class="w-full border rounded p-2" onchange="previewMainEdit(event)">
                    <div class="mt-2"><img id="mainPreviewEdit" class="hidden h-24 w-24 object-cover rounded border" alt="معاينة الصورة الرئيسية"></div>
                    @error('main_image')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
                </div>
                <div class="mb-4">
                    <label class="block mb-1">صور فرعية (يمكن اختيار أكثر من صورة)</label>
                    @if($product->images && $product->images->count())
                        <div class="mb-2 flex flex-wrap gap-3">
                            @foreach($product->images as $img)
                                <div class="relative inline-block">
                                    <img src="{{ asset('storage/'.$img->image_path) }}" alt="صورة فرعية للمنتج {{ $product->name }}" class="h-20 w-20 object-cover rounded border">
                                    <button type="button" class="absolute top-1 left-1 h-7 w-7 rounded-full bg-red-600 text-white flex items-center justify-center shadow-md ring-2 ring-white" title="حذف" onclick="deleteProductImage('{{ route('products.images.destroy', [$product, $img]) }}')">✕</button>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    <input type="file" name="gallery[]" accept="image/*" multiple class="w-full border rounded p-2" onchange="previewGalleryEdit(event)">
                    <div id="galleryPreviewEdit" class="mt-2 flex flex-wrap gap-2"></div>
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 min-w-[160px] justify-center px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-indigo-300">
...
<script>
function previewMainEdit(evt){
  const file = evt.target.files && evt.target.files[0];
  const img = document.getElementById('mainPreviewEdit');
  if(!file){ img.classList.add('hidden'); return; }
  img.src = URL.createObjectURL(file);
  img.classList.remove('hidden');
}
function previewGalleryEdit(evt){
  const files = evt.target.files || [];
  const wrap = document.getElementById('galleryPreviewEdit');
  wrap.innerHTML = '';
...
// إدارة شرائح الأسعار
let priceTiers = {!! old('price_tiers', $product->price_tiers ?? '[]') !!};
...
try {
    const val = document.getElementById('variants_stock_json').value;
    variantsStockData = val ? JSON.parse(val) : [];
} catch(e){}
